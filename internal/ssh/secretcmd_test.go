...
func TestCommandPrompterRunsTheConfiguredCommand(t *testing.T) {
	...
	got, err := p.Password(context.Background(), "s1", hosts.Host{Alias: "web-01"})
	if err != nil {
	...
	got, err = p.Passphrase(context.Background(), "s1", hosts.Host{Alias: "web-01"}, "~/.ssh/id_prod")
	if err != nil {
...
func TestCommandPrompterFallsBackWhenNoCommandIsConfigured(t *testing.T) {
	fallback := FuncPrompter{
		PasswordFunc:   func(context.Context, string, hosts.Host) (string, error) { return "typed", nil },
		PassphraseFunc: func(context.Context, string, hosts.Host, string) (string, error) { return "typed-passphrase", nil },
	...
	if got, err := p.Password(context.Background(), "s1", hosts.Host{Alias: "web-01"}); err != nil || got != "typed" {
		t.Fatalf("Password() = %q, %v", got, err)
	}
	if got, err := p.Passphrase(context.Background(), "s1", hosts.Host{Alias: "web-01"}, "key"); err != nil || got != "typed-passphrase" {
		t.Fatalf("Passphrase() = %q, %v", got, err)
	}
	if got, err := p.Question(context.Background(), "s1", hosts.Host{}, "otp?", true); err != nil || got != "answer to otp?" {
...
func TestCommandPrompterWithoutFallbackOrCommand(t *testing.T) {
	...
	for _, call := range []func() (string, error){
		func() (string, error) { return p.Password(context.Background(), "s1", hosts.Host{Alias: "web-01"}) },
		func() (string, error) {
			return p.Passphrase(context.Background(), "s1", hosts.Host{Alias: "web-01"}, "key")
		},
		func() (string, error) { return p.Question(context.Background(), "s1", hosts.Host{}, "q", false) },
	} {
...
func TestCommandPrompterDoesNotFallBackWhenTheCommandFails(t *testing.T) {
	...
	p := CommandPrompter{
		...
		Fallback: FuncPrompter{PasswordFunc: func(context.Context, string, hosts.Host) (string, error) {
			asked = true
			return "typed", nil
		}},
	}

	if _, err := p.Password(context.Background(), "s1", hosts.Host{Alias: "web-01"}); err == nil {
...
func TestCommandPrompterTimesOut(t *testing.T) {
	...
	start := time.Now()
	_, err := p.Password(context.Background(), "s1", hosts.Host{Alias: "web-01"})
	if err == nil {
